...
function FusionCard({ signal }: { signal: Signal }) {
  const sc = signal.direction_score
  const color = sc <= -2 ? '#ef4444' : sc === -1 ? '#f97316' : sc === 0 ? '#64748b' : '#10b981'
  const bg   = sc <= -2 ? 'rgba(239,68,68,.08)' : sc === -1 ? 'rgba(249,115,22,.08)' : sc === 0 ? 'rgba(100,116,139,.08)' : 'rgba(16,185,129,.08)'
  const label = signal.event_text || signal.signal_id
  const badge = signal.magnitude || (sc <= -2 ? 'HIGH' : sc < 0 ? 'MED' : sc === 0 ? 'FLAT' : 'POS')

  return (
    <div className="rounded-lg border p-3" style={{ background: bg, borderColor: color + '33' }}>
      <div className="flex items-start justify-between gap-2 mb-2">
        <div className="text-xs font-semibold text-gray-200 leading-tight line-clamp-2 flex-1">{label}</div>
        <div className="text-base font-black flex-shrink-0" style={{ color }}>{sc > 0 ? '+' : ''}{sc}</div>
      </div>
      <div className="flex items-center justify-between">
        <span className="text-xs text-gray-600">{signal.source} · {fmtShort(signal.effective_date)}</span>
        <span className="text-xs px-1.5 py-0.5 rounded font-bold" style={{ background: color + '22', color }}>{badge}</span>
      </div>
    </div>
  )
}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-800">
            <div className="font-semibold text-sm text-white">Live Fusion Data</div>
