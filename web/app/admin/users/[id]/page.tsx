...
export default async function AdminUserDetailPage({ params }: { params: { id: string } }) {
  ...
  const objectiveTitles = Array.from(new Set(
    (confidenceScores ?? []).map(c => objectiveMap.get(c.objective_id as string) ?? (c.objective_id as string))
  ))
  ...
  const confidenceHistory = Array.from(dateMap.entries()).map(([date, scores]) => ({ date, ...scores }))
